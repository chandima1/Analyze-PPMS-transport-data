...
filePath1 = 'E:\Transport\Chan2_CEQW241010_Chan3_CEQW241009\Chan2_CEQW241010_Rxx_vs_H_9T__5uA_1.8Kto80K_Chan3_CEQW241009_Rxx_vs_H_9T__5uA_1.8Kto80K_new_00002.dat';
...
ylabel('Symmetrized R_{xx} (Ohms)');   % Add y-axis label
...
set(gca,'color','white')
exportgraphics(gca,'E:\Transport\Chan2_CEQW241010_Chan3_CEQW241009\CEQW241010_Symmetrized_Rxx.tiff','Resolution',600)
...
for j = 1:No_of_temp
    % Get the sorted index
    idx = sort_idx(j);
    plot(Sym_B{idx}(1:half_length), Sym_Rxx3{idx}(1:half_length), '.', 'MarkerSize', 14, 'Color', cmap(j,:));
    legend_entries{j} = ['T = ', num2str(sorted_temperatures(j)), ' K']; % Assign legend entry with sorted temperature
end
axis square; % Set the axes to be square
xlabel('Magnetic Field (T)'); % Add x-axis label
...
yline(currentYLim(2),'k','LineWidth',2,'HandleVisibility', 'off')
set(gca,'color','white')
exportgraphics(gca,'E:\Transport\Chan2_CEQW241010_Chan3_CEQW241009\CEQW241009_Symmetrized_Rxx.tiff','Resolution',600)
...
for j = 1:No_of_temp
    ...
    for k = 1:length(Sym_B{idx}) % Loop through the magnetic field data
        ...
        if k <= length(Sym_B{idx}) && k <= length(Sym_Rxx2{idx})
...
combined_table = cell2table(combined_data, 'VariableNames', {'Temperature', 'MagneticField', 'Symmetrized_Rxx_Chan2', 'Symmetrized_Rxx_Chan3'});

% Write to CSV
writetable(combined_table, 'E:\Transport\Chan2_CEQW241010_Chan3_CEQW241009\Symmetrized_Rxx_9T_Chan2_CEQW241010_Chan3_CEQW241009.csv');
